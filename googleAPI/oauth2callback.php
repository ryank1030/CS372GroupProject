...
$client->setRedirectUri('http://' . $_SERVER['HTTP_HOST'] . '/oauth2callback.php');  //set redirect url
...
if (! isset($_GET['code'])) {  //if we don't have the code yet
  ...
  $redirect_uri = 'http://' . $_SERVER['HTTP_HOST'] . '/signin-addEvent.php';  //redirect var to host address
  header('Location: ' . filter_var($redirect_uri, FILTER_SANITIZE_URL));  //go back to host address
}
?>