...
class User
{
	...
	public function get_user($db, $googleID_in)
	{
		...
		if (!$result = $db->query($uSql->get_user_id($googleID_in)))	// ### note: ifs like this check if a result is returned and if the assignment operation goes through. If it doesn't, then that indicates that either the connection is dead or the query returns nothing.
		{
			return false;
		} 
		...
		if ($result2 = $db->query($uSql->get_user($this->$user_id)))
		{
			while ($row = mysqli_fetch_row($result)) 
			{					
	...
	public function get_user_basic($db, $user_id_in)
	{
		// get user info
		// will return false if the user_id_in doesn't exist
		if (!$result = $db->query($uSql->get_user_basic($user_id_in)))
		{
			return false;
		} 
		...
		if ($result = $db->query($uSql->get_user($this->$user_id)))
		{
			while ($row = mysqli_fetch_row($result)) 
			{					
				$this->$email = $row["email"];
	...
	public function update_last_login($db)
	{
		...
		if (!$result = $db->query($uSql->user_update_login_time($this->$user_id)))
		{
			return false;
		} 
		...
		if ($result = $db->query($uSql->user_last_login($this->$user_id)))
		{
			while ($row = mysqli_fetch_row($result)) 
			{					
				$this->$user_id = $row[0];
	...
	public function create_user($db)
	{
		...
		if (!$result = $db->query($uSql->get_user_id($google_id)))
		{
			return false;
		} 
		
		// generate a new user_id based on the last max user_id on the Users table.
		if ($result2 $db->query($uSql->max_user_id()))
		{	
			while ($row = mysqli_fetch_row($result)) 
			{					
				$old_id = $row[0];
	...
	public function edit_user($db, $email_in, $first_name_in, $last_name_in, $dob_in, $image_url_in,  $phone_in)
	{
		set_user($email_in, $first_name_in, $last_name_in, $dob_in, $image_url_in,  $phone_in);
		
		if (!$result = $db->query($uSql->update_user($user_id, $email, $first_name, $last_name, $dob, $image_url, $phone)))
		{
			return false;
		} 
	...
	{
		if (!$result = $db->query($uSql->delete_user($user_id)))
		{
			return false;
		} 
